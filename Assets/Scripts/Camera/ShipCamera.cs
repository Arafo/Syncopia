...
public class ShipCamera : ShipCore {
    ...
    private void UpdateCamera()
    {
        ...
        if (cameraMode == 0) {
            ...
            cameraInputAmount = Mathf.Lerp(cameraInputAmount, -transform.InverseTransformDirection(ship.body.velocity).x, Time.deltaTime * ship.config.cameraSpeed);
            //
            ...
            cameraFallLag = Mathf.Lerp(cameraFallLag, transform.InverseTransformDirection(ship.body.velocity).y * (0.05f - cameraFallHelper), Time.deltaTime * ship.config.cameraSpeed);
            ...
            cameraAccelLag = Mathf.Lerp(cameraAccelLag, -(transform.InverseTransformDirection(ship.body.velocity).z * Time.deltaTime) * 0.25f, Time.deltaTime * ship.config.cameraSpeed);
            ...
            else if (ship.input.m_LeftAirBrakeAxis != 0f && ship.input.m_RightAirBrakeAixs != 0f && transform.InverseTransformDirection(ship.body.velocity).z > 10f) {
            ...
            float value = Mathf.Abs(transform.InverseTransformDirection(ship.body.angularVelocity).x) * 0.5f;
            value = Mathf.Clamp(value, 0.1f, float.PositiveInfinity);
            float num = transform.InverseTransformDirection(ship.body.velocity).z * 0.04f;
            num = Mathf.Clamp(num, 0f, 16f);
            cameraPitchMod = Mathf.Lerp(cameraPitchMod, transform.InverseTransformDirection(ship.body.angularVelocity).x * num, Time.deltaTime * ship.config.cameraSpeed);
            float x = cameraOffset.x * 0.1f;
            Vector3 a = transform.TransformPoint(x, -4f, 25f + cameraPitchMod + cameraMovementLength);
            //transform.rotation = Quaternion.LookRotation(a - transform.position, transform.up);
        }
